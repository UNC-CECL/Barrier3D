...
def makeStormSet():
    ...
    for t in range(StormStart, 1000):
        ...
        for n in range(numstorm):
            storm = random.randint(
                0, len(StormList) - 1
            )  # Pick random row index of each storm in list
...
def makeDuneDomain():
    Dstart = np.random.uniform(
        (1 / 10), (Dmaxel - BermEl - (0.1 / 10))
    )  # Random start height
...
def RunBatch(n):
    ...
    for g in range(len(Rmin)):
        ...
        print("=================================")
        print(
            "Shrub = ", Shrub_ON, ", AvgGrowthRate = ", Grate, ", Batch Run ", SimNumber
        )
        (
            Result,
            t_Result,
