...
def SeaLevelBatch(InteriorDomain, DuneDomain, t, rslr):
...
def FindWidths(InteriorDomain, SL):
    ...
    for l in range(BarrierLength):
        width = next(
            (index for index, value in enumerate(InteriorDomain[:, l]) if value <= SL),
            DomainWidth,
        )
        width = width - 1
        if width < 0:
...
def LTA_SC(
    ...
    x_s_dt = 2 * (Qow + Qdg + Qat) / ((2 * h_b) + d_sf) - (
...
def Shrubs(
    ...
    BeachWidth = (
        50 / 10
    )  # Calculated based on berm (dune toe) elevation of 1.9, and beach slope of 0.04 (i.e., 1.9/0.04 = 47.5 ~= 50 m)

    ### Burial / Uprooting
    ShrubHeight = ShrubPercentCover * MaxShrubHeight
    DeadHeight = DeadPercentCover * MaxShrubHeight

    # # Kill shrubs buried past threshold
    # ShrubDomainDead[BurialDomain > (BurialLimit * ShrubHeight)] = ShrubDomainAll[BurialDomain > (BurialLimit * ShrubHeight)]
    # ShrubDomainFemale[BurialDomain > (BurialLimit * ShrubHeight)] = 0
    # ShrubDomainMale[BurialDomain > (BurialLimit * ShrubHeight)] = 0

    # # Remove shrubs completely buried or uprooted
    # ShrubDomainFemale[BurialDomain > ShrubHeight] = 0
    # ShrubDomainMale[BurialDomain > ShrubHeight] = 0
    # ShrubDomainFemale[BurialDomain < UprootLimit] = 0
    # ShrubDomainMale[BurialDomain < UprootLimit] = 0

    # # Remove dead shrubs completely buried or uprooted
    # ShrubDomainDead[BurialDomain > DeadHeight] = 0
    # ShrubDomainDead[BurialDomain < UprootLimit] = 0

    # # Reset burial domain
    # tempAll = ShrubDomainFemale + ShrubDomainMale + ShrubDomainDead
    # BurialDomain[tempAll == 0] = 0
    # ShrubDomainAll = ShrubDomainFemale + ShrubDomainMale # Recalculate

    # Alternative method with loops
    for l in range(len(ShrubPercentCover[0])):
        for w in range(len(ShrubPercentCover)):
            if ShrubDomainAll[w, l] > 0:
                if (
                    BurialDomain[w, l] > ShrubHeight[w, l]
                    or BurialDomain[w, l] < UprootLimit
                ):
                ...
                if (
                    BurialDomain[w, l] > DeadHeight[w, l]
                    or BurialDomain[w, l] < UprootLimit
                ):
            ...
            if InteriorDomain[w, l] < ShrubEl_min:
                if InteriorDomain[w, l] > SL:
                    if ShrubDomainFemale[w, l] > 0 or ShrubDomainMale[w, l] > 0:
                        ShrubDomainDead[w, l] = (
                            ShrubDomainFemale[w, l] + ShrubDomainMale[w, l]
    ...
    if (
        ...
        and DuneDomainCrest[randX] + BermEl >= Dshrub
        and InteriorDomain[randY, randX] >= ShrubEl_min
    ...
    for k in range(
        ...
        if 0 in ShrubDomainAll:
            ...
            I = [
                index for index, value in enumerate(FemaleShrubs) if value >= TimeFruit
            ]
            numShrubCells = len(I)
            ...
            for i in range(numShrubCells):
                ...
                if Survivors > 0:
                    DispDist = np.round(
                        np.random.lognormal(disp_mu, disp_sigma, Survivors)
                    )
                else:
                    DispDist = []
                ...
                if len(DispDist) > 0:
                    for j in range(
                        ...
                        if DispDist[j] > 0:
                            ...
                            distMat = np.round(
                                np.sqrt((X - matOriginX) ** 2 + (Y - matOriginY) ** 2)
                            ...
                            if (
                                ...
                                and targetX >= 0
                                and targetX < BarrierLength
                                and ShrubDomainFemale[targetY, targetX] == 0
                                and ShrubDomainMale[targetY, targetX] == 0
                                and InteriorDomain[targetY, targetX] >= ShrubEl_min
                                and InteriorDomain[targetY, targetX] <= ShrubEl_max
                                and ShrubDomainDead[targetY, targetX] < 1
                            ):
                                if (
                                    DuneDomainCrest[targetX] + BermEl >= Dshrub
                                    ...
                                    BeachWidth + DuneWidth + targetY > SprayDist
...
def CalcPC(
    ShrubDomainAll, PercentCoverTS, ShrubDomainDead, DeadPercentCoverTS, ShrubArea, t
):
...
def plot_ElevAnimation(
    ...
    for t in range(TMAX):
        ...
        if not os.path.exists(newpath):
            os.makedirs(newpath)
        plt.text(1, 1, timestr)
        elevFig1.subplots_adjust(right=0.85)
...
def plot_DuneStorm(Hd_AverageTS, StormSeries, TMAX):
    ...
    # Dune Height
    aHd = [a * 10 for a in Hd_AverageTS]
    plt.plot(aHd, color="mediumblue")
    plt.xlabel("Year")
    plt.ylabel("Avg. Dune Height (m)")
    ...
    stormX = StormSeries[0:stop, 0]
    stormY = StormSeries[0:stop, 1]

    stormX = stormX[stormY > BermEl]
    stormY = stormY[stormY > BermEl]
    stormY = [(a - BermEl) * 10 for a in stormY]
...
def plot_SeabedProfile(SL, TMAX, x_t_TS):
    ...
    for t in range(TMAX):
        ...
        Ty.append(((SL + (t * RSLR)) - DShoreface) * 10)
        Sbx.append(xavg * t * 10)
        Sby.append((-DShoreface + (t * RSLR)) * 10)
...
def plot_ShrubFront(Shrub_ON, TMAX, PercentCoverTS):
    ...
    if Shrub_ON:
        ...
        for t in range(TMAX):
            ...
            ShrubTotal = np.count_nonzero(
                shrub
            )  # Total amount of shrub plants in given year
