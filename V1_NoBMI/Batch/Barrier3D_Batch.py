...
def Barrier3D_Batch(SimNumber, directory, StormSeries, growthparam):
    ...
    if Shrub_ON == 1:
        np.savez(filename, DuneDomain = DuneDomain, DomainTS = DomainTS, x_s_TS = x_s_TS, x_b_TS = x_b_TS, x_t_TS = x_t_TS, s_sf_TS = s_sf_TS, InteriorWidth_AvgTS = InteriorWidth_AvgTS, QowTS = QowTS, QsfTS = QsfTS, Hd_AverageTS = Hd_AverageTS, 
             PercentCoverTS = PercentCoverTS, DeadPercentCoverTS = DeadPercentCoverTS, ShrubArea = ShrubArea, ShrubDomainAll = ShrubDomainAll, ShrubDeadTS = ShrubDeadTS, StormCount = StormCount, t = t, RunUpCount = RunUpCount, AvgInteriorElevationTS = AvgInteriorElevationTS,
             IslandAreaTS = IslandAreaTS, InundationCount = InundationCount, ShorelineChange = ShorelineChange, Hd_Loss_TS = Hd_Loss_TS, SimDuration = SimDuration, StormSeries = StormSeries, Dmax = Dmax, SL = SL, MaxAvgSlope = MaxAvgSlope, fluxLimit = fluxLimit, SimParams = SimParams)
    else: 
        np.savez(filename, DuneDomain = DuneDomain, DomainTS = DomainTS, x_s_TS = x_s_TS, x_b_TS = x_b_TS, x_t_TS = x_t_TS, s_sf_TS = s_sf_TS, InteriorWidth_AvgTS = InteriorWidth_AvgTS, QowTS = QowTS, QsfTS = QsfTS, Hd_AverageTS = Hd_AverageTS, 
             ...
             AvgInteriorElevationTS = AvgInteriorElevationTS, AvgInteriorElevation = AvgInteriorElevation, IslandAreaTS = IslandAreaTS, MaxAvgSlope = MaxAvgSlope, fluxLimit = fluxLimit, SimParams = SimParams)
    
    

    return Result, t_Result, InteriorWidth_Avg, ShorelineChange, Hd_avg, IslandArea, AvgInteriorElevation, SCperiod, AvgFastDur, AvgSlowDur, Punc
