...
def Barrier3D_BatchShrub(
    SimNumber, directory, StormSeries, growthparam, Shrub_ON, RSLR, DuneDomain, Dstart
):
    ...
    for t in range(1, TMAX):  # Yearly time steps - actual time = t + 1
        ...
        ### RSLR
        InteriorDomain, DuneDomain = func.SeaLevelBatch(
            InteriorDomain, DuneDomain, t, RSLR
        )

        ### Find DomainWidth and InteriorWidth
        DomainWidth, InteriorWidth, InteriorWidth_Avg = func.FindWidths(
            InteriorDomain, SL
        )
        ...
        DuneDomainCrest = DuneDomain[t, :, :].max(
            axis=1
        ...
        if t >= StormStart:
            ...
            if StormTimeSeries == True:
                ...
                numstorm = round(
                    np.random.normal(mean_storm, SD_storm)
            ...
            if numstorm > 0:
                ...
                for n in range(numstorm):  # Loop through each individual storm
                    ...
                    gaps = func.DuneGaps(
                        DuneDomainCrest, Dow, BermEl, Rhigh[n]
                    ...
                    for q in range(len(gaps)):
                        ...
                        ) + BermEl  # Average elevation of dune gap
                    ...
                    if (
                        len(gaps) > 0 and Iow / len(gaps) >= threshold_in
                    ):  # If greater than threshold % of dune gaps are in unundation regime, use inundation regime routing
                        inundation = 1
                        substep = OWss_i
                        InundationCount += 1
                    else:
                    ...
                    width = (
                        np.shape(InteriorDomain)[0] + 1 + add
                    ...
                    for q in range(len(gaps)):
                        ...
                        if inundation == 1:  # Inundation regime
                            ...
                            AvgSlope = (
                                BermEl / 20
                            )  # InteriorWidth_Avg #<-------------------TEMP!!! Hold C constant
                    ...
                    for TS in range(duration):
                        ...
                        for d in range(width - 1):
                            ...
                            for i in range(BarrierLength):
                                if Discharge[TS, d, i] > 0:
                                    ...
                                    if Elevation[TS, d, i] > SL or any(
                                        z > SL for z in Elevation[TS, d + 1 : d + 6, i]
                                    ):
                                        if i > 0:
                        ...
                        BurialDomain = func.UpdateBurial(
                            BurialDomain,
                            ElevationChange,
                            ShrubDomainWidth,
                            ShrubDomainAll,
        ...
        SCR, x_s, x_t, x_s_TS, x_t_TS, x_b_TS, s_sf_TS, QowTS, QsfTS = func.LTA_SC(
            InteriorDomain,
            OWloss,
            Qdg,
            DuneLoss,
        ...
        if abs(SCRagg) >= 1:
            ...
            if (
                ...
                for d in range(sc):
                    InteriorDomain = np.vstack(
                        [DuneDomain[t, :, -1] + BermEl, InteriorDomain]
                    ...
                    if StormTimeSeries == 0:
                        newDuneHeight = np.ones([BarrierLength]) * (
                            ...
                            + (
                                -0.005
                                + (0.005 - (-0.005)) * np.random.rand(BarrierLength)
                            )
                        )
                    else:
                    ...
                    ShrubDomainFemale = np.concatenate(
                        (np.zeros([1, BarrierLength]), ShrubDomainFemale)
                    )
                    ShrubDomainMale = np.concatenate(
                        (np.zeros([1, BarrierLength]), ShrubDomainMale)
                    )
                    ShrubDomainDead = np.concatenate(
                        (np.zeros([1, BarrierLength]), ShrubDomainDead)
                    )
                    ShrubPercentCover = np.concatenate(
                        (np.zeros([1, BarrierLength]), ShrubPercentCover)
                    )
                    DeadPercentCover = np.concatenate(
                        (np.zeros([1, BarrierLength]), DeadPercentCover)
                    )
                    BurialDomain = np.concatenate(
                        (np.zeros([1, BarrierLength]), BurialDomain)
                    )
    ...
    SCperiod, AvgFastDur, AvgSlowDur, Punc = func.calc_ShorelinePeriodicity(
        TMAX, x_s_TS
    )
    ...
    if Shrub_ON == 1:
        np.savez(
            filename,
            DuneDomain=DuneDomain,
            DomainTS=DomainTS,
            x_s_TS=x_s_TS,
            x_b_TS=x_b_TS,
            x_t_TS=x_t_TS,
            s_sf_TS=s_sf_TS,
            InteriorWidth_AvgTS=InteriorWidth_AvgTS,
            QowTS=QowTS,
            QsfTS=QsfTS,
            Hd_AverageTS=Hd_AverageTS,
            PercentCoverTS=PercentCoverTS,
            DeadPercentCoverTS=DeadPercentCoverTS,
            ShrubArea=ShrubArea,
            ShrubDomainAll=ShrubDomainAll,
            ShrubDeadTS=ShrubDeadTS,
            StormCount=StormCount,
            t=t,
            RunUpCount=RunUpCount,
            InundationCount=InundationCount,
            ShorelineChange=ShorelineChange,
            Hd_Loss_TS=Hd_Loss_TS,
            SimDuration=SimDuration,
            StormSeries=StormSeries,
            Dmax=Dmax,
            SL=SL,
            InteriorWidth_Avg=InteriorWidth_Avg,
            Hd_avg=Hd_avg,
            IslandArea=IslandArea,
            t_Result=t_Result,
            Result=Result,
            SCperiod=SCperiod,
            AvgFastDur=AvgFastDur,
            AvgSlowDur=AvgSlowDur,
            Punc=Punc,
            AvgInteriorElevationTS=AvgInteriorElevationTS,
            AvgInteriorElevation=AvgInteriorElevation,
            IslandAreaTS=IslandAreaTS,
            MaxAvgSlope=MaxAvgSlope,
            fluxLimit=fluxLimit,
            Dstart=Dstart,
            RSLR=RSLR,
            growthparam=growthparam,
            SimParams=SimParams,
        )
    else:
        np.savez(
