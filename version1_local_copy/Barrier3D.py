...
SCRagg = (
    0  # Counter variable for shoreline change that is less than 1 cell size per year
)
ShorelineChangeTS = [0]
    ...
    if t >= StormStart:
        ...
        if numstorm > 0:
            ...
            for n in range(numstorm):  # Loop through each individual storm
                ...
                )  # (dam) Add one for Dunes and 10 for bay
                ...
                for TS in range(duration):
                    ...
                    BurialDomain = func.UpdateBurial(
                        BurialDomain, ElevationChange, ShrubDomainWidth, ShrubDomainAll
                    )
            ...
            for d in range(sc):
                ...
                if StormTimeSeries == 0:
                    newDuneHeight = np.ones([BarrierLength]) * (
                        0.005
                        + (-0.005 + (0.005 - (-0.005)) * np.random.rand(BarrierLength))
                    )
                else:
...
# func.plot_ElevTMAX(
#     TMAX, t, DuneDomain, DomainTS, Shrub_ON, PercentCoverTS, DeadPercentCoverTS
# )
#
#
