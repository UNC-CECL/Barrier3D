...
def set_barrier3d_yaml(
        ...
        berm_el=1.9,
        # [m NAVD88] Static elevation of berm; berm elevation + dune height = dune elevation (NOTE: if changed, need new MSSM and storms)
    ...
    set_yaml("BarrierLength", barrier_len, fid)  # [m] Static length of island segment (comprised of 10x10 cells)
    ...
    set_yaml('MHW', MHW, datadir)
    # (NOTE: if MHW is changed, the storm time series needs to be remade)
    set_yaml("DuneParamStart", dune_param_start, fid)  # Dune height will come from external file
    ...
    set_yaml("GrowthParamStart", growth_param_start, fid)  # Dune growth parameter WILL NOT come from external file
    ...
    set_yaml("beta", beta, datadir)
    set_yaml("StormSeries", storm_series, fid)
    set_yaml("nn", nn, fid)
    set_yaml("mm", mm, fid)
...
def run_changing_dune_params(rmin, rmax, datadir):
    set_barrier3d_yaml(rmin, rmax, datadir)
    a_barrier3d_model = Barrier3d.from_yaml(datadir)

    # increase time step
    for time_step in range(1, a_barrier3d_model.TMAX):
        a_barrier3d_model.update()  # update the model by a time step
...
run_changing_dune_params(rmin=0.45, rmax=0.85, datadir=datadir)
