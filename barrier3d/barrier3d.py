...
class Barrier3D:
    def SeaLevel(self, InteriorDomain, DuneDomain, t):
        ...
        InteriorDomain[InteriorDomain < -self._BayDepth] = -self._BayDepth # Bay can't be deeper than BayDepth (assumes equilibrium depth maintained)
    ...
    def DuneGrowth(self, DuneDomain, t):
        ...
        for q in range(self._DuneWidth):
            reduc = 1/(Cf**q)
            G = self._growthparam * DuneDomain[t-1,:,q] * (1 - DuneDomain[t-1,:,q] / Dmax) * reduc
            DuneDomain[t,:,q] = G + DuneDomain[t-1,:,q]
            Qdg = Qdg + (np.sum(G) / self._BarrierLength) # Volume of sediment lost from beach/shoreface from dune growth
        
        return DuneDomain, Dmax, Qdg

    def Shrubs(self, InteriorDomain, DuneDomainCrest, t, ShrubDomainFemale, ShrubDomainMale, BurialDomain, InteriorWidth_Avg, DomainWidth, ShrubDomainDead):
        ...
        ShrubDomainDead[BurialDomain > self._BurialLimit] = ShrubDomainAll[BurialDomain > self._BurialLimit] # Transfer to dead domain
        ShrubDomainDead[BurialDomain >= 0.2] = 0 # If buried by more than 2 m, considered shrub to be completely gone
        ShrubDomainDead[BurialDomain < self._UprootLimit] = ShrubDomainAll[BurialDomain < self._UprootLimit] # Transfer to dead domain 
        ...
        if  ShrubDomainFemale[randY,randX] == 0 and ShrubDomainMale[randY,randX] == 0 and DuneDomainCrest[randX] + self._BermEl >= self._Dshrub and InteriorDomain[randY,randX] >= self._ShrubEl_min and InteriorDomain[randY,randX] <= self._ShrubEl_max:
        ...
        ### Disperse seeds
        for k in range(self._BarrierLength): # Loop through each row of island width (i.e. from ocean to mainland side of island)
            if 0 in ShrubDomainAll:                  #<--------------- Working??            
              
                # For all cells with a shrub
                FemaleShrubs = ShrubDomainFemale[:,k]
                I = [index for index, value in enumerate(FemaleShrubs) if value >= self._TimeFruit]
                numShrubCells = len(I)
                # Determine how many seeds in each cell
                Seedspercell = np.random.randint(self._Seedmin,high = self._Seedmax, size = numShrubCells)
                ...
                for i in range(numShrubCells):
                    ...
                    if Survivors > 0:
                        DispDist = np.round(np.random.lognormal(self._disp_mu,self._disp_sigma,Survivors))
                    else:
                        DispDist = []
                    ...
                    if len(DispDist) > 0:
                        for j in range(len(DispDist+1)): # Loop through each individual seed to disperse
                            ...
                            if DispDist[j] > 0:
                                ...
                                if  targetY >= 0 and targetY < DomainWidth and targetX >= 0 and targetX < self._BarrierLength and ShrubDomainFemale[targetY,targetX] == 0 and ShrubDomainMale[targetY,targetX] == 0 \
                                    and DuneDomainCrest[targetX] + self._BermEl >= self._Dshrub and InteriorDomain[targetY,targetX] >= self._ShrubEl_min and InteriorDomain[targetY,targetX] <= self._ShrubEl_max and ShrubDomainDead[targetY,targetX] < 1:
        ...
        for w in range(self._DuneWidth):
            ...
            for d in range(2,self._BarrierLength): # Loop L to R
                ...
                if Ldiff > self._HdDiffu:
                    sub = (Ldiff - self._HdDiffu)/2 # Height difference in excess of maximum, divided by two
    ...
    def SalineFlooding(self, ShrubDomainWidth, ShrubDomainAll, ShrubDomainFemale, ShrubDomainMale, ShrubDomainDead, d, i, Q0):
    ...
    def UpdateShrubDomains(self, DomainWidth, ShrubDomainWidth, ShrubDomainFemale, ShrubDomainMale, ShrubDomainAll, ShrubPercentCover, BurialDomain, ShrubDomainDead):
    ...
    def LTA_SC(self, InteriorDomain, OWloss, Qdg, DuneLoss, x_s, x_s_TS, x_t, x_t_TS, x_b_TS, s_sf_TS, InteriorWidth_Avg, SL, QowTS, QsfTS):
        ...
        Qow = (OWloss) / (self._BarrierLength) # (dam^3/dam) Volume of sediment lost from shoreface/beach by overwash
        ...
        # Shoreface Flux
        s_sf = d_sf / (x_s - x_t)
        Qsf = (self._k_sf / 100) * (self._s_sf_eq - s_sf) # Convert Ksf from m^3/m/yr to dam^3/dam/yr
        QsfTS.append(Qsf * 100) # Save in m^3/m    

        # Toe, Shoreline, and island base elevation changes     
        x_t_dt = (4 * Qsf * (h_b + d_sf) / (d_sf * (2 * h_b + d_sf))) + (2 * self._RSLR / s_sf)   
        x_s_dt = 2 * (Qow + Qdg + self._Qat) / ((2 * h_b) + d_sf) - (4 * Qsf * (h_b + d_sf) / (((2 * h_b) + d_sf)**2)) # Dune growth and alongshore transport added to LTA14 formulation
    ...
    def __init__(self, **kwds):
        ...
        self._DomainTS = [None] * self._TMAX # Stores the elevation domain for each timestep
        ...
        self._x_b_TS = [(self._x_s + self._DomainWidth)] # (dam) Bay shoreline locations for each time step
    ...
    def update(self):
        ### RSLR
        self._InteriorDomain, self._DuneDomain = self.SeaLevel(self._InteriorDomain, self._DuneDomain, self._time_index)

        ### Find DomainWidth and InteriorWidth
        self._DomainWidth, InteriorWidth, InteriorWidth_Avg = self.FindWidths(self._InteriorDomain, self._SL)
        ...
        if InteriorWidth_Avg < 1: # if max(InteriorWidth) <= 1:
            print('Barrier has WIDTH DROWNED at t = ' + str(self._time_index) + ' years!')
            self._TMAX = self._time_index-1
            return
        if all(j <= self._SL for j in self._InteriorDomain[0,:]):
            print('Barrier has HEIGHT DROWNED at t = ' + str(self._time_index) + ' years!')
            self._TMAX = self._time_index-1
            return


        ### Grow Dunes
        self._DuneDomain, Dmax, Qdg = self.DuneGrowth(self._DuneDomain, self._time_index)

        DuneDomainCrest = self._DuneDomain[self._time_index,:,:].max(axis=1) # Maximum height of each row in DuneDomain
        self._Hd_AverageTS.append(np.mean(DuneDomainCrest)) # Store average pre-storms dune-heigh for time step
        ...
        if self._Shrub_ON == 1:
            ...
            ShrubDomainAll, self._ShrubDomainFemale, self._ShrubDomainMale, self._BurialDomain = self.Shrubs(self._InteriorDomain, DuneDomainCrest, self._time_index, self._ShrubDomainFemale, self._ShrubDomainMale, self._BurialDomain, InteriorWidth_Avg, self._DomainWidth)
        ...
        if self._time_index >= self._StormStart:
            ...
            self._numstorm = round(np.random.normal(self._mean_storm, self._SD_storm)) # analysis:ignore # Comment out this line if using pre-specified static number of storms per year
            ...
            if self._numstorm > 0:
                ...
                for n in range(self._numstorm): # Loop through each individual storm
                    ...
                    DuneChange = np.zeros([self._BarrierLength, self._DuneWidth]) # Vector storing dune height change for this storm
                    
                    # Find overwashed dunes and gaps           
                    Dow = [index for index, value in enumerate((DuneDomainCrest + self._BermEl)) if value < Rhigh[n]]
                    gaps = self..DuneGaps(DuneDomainCrest, Dow, self._BermEl, Rhigh[n]) # Finds location and Rexcess of continuous gaps in dune ridge                
                    
                    for d in range(len(Dow)): # Loop through each overwashed dune cell
                        for w in range(self._DuneWidth):

                            # Calculate dune elevation loss
                            Rnorm = Rhigh[n] / (self._DuneDomain[self._time_index, Dow[d], w] + self._BermEl) # Rhigh relative to pre-storm dune elevation
                            Dloss = Rnorm / (self._C1 + (Rnorm * (Rnorm - self._C2))) # Amount of dune crest elevation change normalized by pre-storm dune elevation (i.e. a percent change), from Goldstein and Moore (2016)
                            
                            # Set new dune height
                            InitDElev = self._DuneDomain[self._time_index, Dow[d], w] + self._BermEl
                            NewDElev = InitDElev * (1-Dloss) # Calculate new dune elevation from storm lowering
                            if NewDElev < self._BermEl:
                                NewDElev = self._BermEl
                            self._DuneDomain[self._time_index, Dow[d], w] = NewDElev - self._BermEl # Convert elevation to height above berm
                            ...
                            if self._DuneDomain[self._time_index, Dow[d], w] < self._DuneRestart:
                                if self._DuneRestart < Dmax:
                                    self._DuneDomain[self._time_index, Dow[d], w] = self._DuneRestart
                                else:
                                    self._DuneDomain[self._time_index, Dow[d], w] = Dmax # Restart height can't be greater than Dmax

                    # Dune Height Diffusion
                    self._DuneDomain = self.DiffuseDunes(self._DuneDomain, self._time_index)
                    ...
                    for q in range(len(gaps)):
                        ...
                        meandune = (sum(Dunes_prestorm[start:stop+1]) / gapwidth) + self._BermEl # Average elevation of dune gap
                    ...
                    if len(gaps) > 0 and Iow / len(gaps) >= self._threshold_in: # If greater than 25% of dune gaps are in unundation regime, use inundation regime routing
                        inundation = 1
#                        #### Calc substep
#                        Smax = 0
#                        Smin = 0
...
#                        ####
                    ...
                    width = np.shape(self._InteriorDomain)[0] + 1 + add # (dam) Add one for Dunes and 25 for bay
                        ...
                        if inundation == 1: # Inundation regime       
                            ...
                            for u in range(1,width-1):
                                for v in range(self._BarrierLength):                            
                                    if Elevation[0,u,v] > self._SL and Elevation[0,u+1,v] > self._SL:
                                        SS = Elevation[0,u,v] - Elevation[0,u+1,v]
                                        slopes.append(SS)
                            AvgSlope = sum(slopes) / len(slopes)
                            C = 2 * AvgSlope                             
                            # Set sediment at dune gap
                            SedFluxIn[:,0,start:stop] = self._Ki * (Qdune*(AvgSlope + C))**self._mm        
                        else: # Run-up regime
                            Rin = self._Rin_r
                            # Set sediment at dune gap
                            ...
                            for i in range(self._BarrierLength):                                       
                                if Discharge[TS,d,i] > 0:
                                    ...
                                    if S1 > 0 or S2 > 0 or S3 > 0:
                                        ...
                                        Q1 = Q0 * S1**self._nn / (S1**self._nn + S2**self._nn + S3**self._nn)
                                        Q2 = Q0 * S2**self._nn / (S1**self._nn + S2**self._nn + S3**self._nn)
                                        Q3 = Q0 * S3**self._nn / (S1**self._nn + S2**self._nn + S3**self._nn)
                                    ...
                                    # All slopes negative
                                        ...
                                        Q1 = Q0 * abs(S1)**(-self._nn) / (abs(S1)**(-self._nn) + abs(S2)**(-self._nn) + abs(S3)**(-self._nn))
                                        Q2 = Q0 * abs(S2)**(-self._nn) / (abs(S1)**(-self._nn) + abs(S2)**(-self._nn) + abs(S3)**(-self._nn))
                                        Q3 = Q0 * abs(S3)**(-self._nn) / (abs(S1)**(-self._nn) + abs(S2)**(-self._nn) + abs(S3)**(-self._nn))                                  
                                    ...
                                    if self._Shrub_ON == 1:             
                                        ...
                                        if i > 0:
                                            if d < (ShrubDomainWidth):
                                                if self._ShrubPercentCover[d,i-1] > 0: 
                                                    Q1 =  Q1 * (self._Qshrub_max * self._ShrubPercentCover[d,i-1])                                    
                                            Discharge[TS,d+1,i-1] = Discharge[TS,d+1,i-1] + Q1
                                            
                                        # Cell 2                                    
                                        if d < (ShrubDomainWidth - 1):
                                            if self._ShrubPercentCover[d,i] > 0:
                                                Q2 =  Q2 * (self._Qshrub_max * self._ShrubPercentCover[d,i])
                                        ...
                                        if i < (self._BarrierLength-1):
                                            if d < (ShrubDomainWidth):
                                                if self._ShrubPercentCover[d,i+1] > 0:
                                                    Q3 =  Q3 * (self._Qshrub_max * self._ShrubPercentCover[d,i+1])                                         
...
#                                            Qs3 = 0
                                    ...
                                    if Elevation[TS,d,i] > self._SL: # If cell is subaerial, elevation change is determined by difference between flux in vs. flux out
                                        if i > 0:
                                        ...
                                        if inundation == 1: # Inundation regime
                                            ...
                                            SedFluxOut[TS,d,i] = SedFluxIn[TS,d,i] * self._Cbb_i
                                            ...
                                            if i < (self._BarrierLength-1) and Qs1 > Qs_bb_min:
                                                SedFluxIn[TS,d+1,i+1] += Qs3
                                        
                                        else: # Run-up regime

                                            SedFluxOut[TS,d,i] = SedFluxIn[TS,d,i] * self._Cbb_r
                                            Qs0 = SedFluxOut[TS,d,i]
                                    ...
                                    ### Saline Flooding 
                                    self._ShrubDomainFemale, self._ShrubDomainMale = self.SalineFlooding(ShrubDomainWidth, ShrubDomainAll, self._ShrubDomainFemale, self._ShrubDomainMale, d, i, Q0)  
                        ...
                        self._BurialDomain = self.UpdateBurial(self._BurialDomain, ElevationChange, ShrubDomainWidth, ShrubDomainAll)
                    ...
                    self._ShrubDomainFemale, self._ShrubDomainMale, ShrubDomainAll, self._ShrubPercentCover, self._BurialDomain = self.UpdateShrubDomains(self._DomainWidth, ShrubDomainWidth, self._ShrubDomainFemale, self._ShrubDomainMale, ShrubDomainAll, self._ShrubPercentCover, self._BurialDomain)
        ...
        SCR, self._x_s, self._x_t, self._x_s_TS, self._x_t_TS, self._x_b_TS, self._s_sf_TS, self._QowTS, self._QsfTS = self.LTA_SC(self._InteriorDomain, OWloss, self._x_s, self._x_s_TS, self._x_t, self._x_t_TS, self._x_b_TS, self._s_sf_TS, InteriorWidth_Avg, self._SL, self._QowTS, self._QsfTS)

        
        ### Erode/Prograde Ocean Shoreline
        drown_break = 0
             
        self._SCRagg = self._SCRagg + SCR # Account for any residual shoreline change (less than cell size) from previous time step        
        
        if abs(self._SCRagg) >= 1:
            sc = math.floor(abs(self._SCRagg))
            if self._SCRagg > 0: # Positive = prograde, add row(s) to front of interior domain
                for d in range(sc):
                    self._InteriorDomain = np.vstack([self._DuneDomain[self._time_index,:,-1] + self._BermEl, self._InteriorDomain]) # New interior row added with elevation of previous dune field (i.e. previous dune row now part of interior)
                    
                    newDuneHeight = np.ones([self._BarrierLength]) * (0.01 + (-0.005 + (0.005 - (-0.005)) * np.random.rand(self._BarrierLength)))
                    self._DuneDomain[self._time_index,:,:] = np.roll(self._DuneDomain[self._time_index,:,:], 1, axis=1)
                    self._DuneDomain[self._time_index,:,0] = newDuneHeight
                    
                    # Update shrub domains too
                    self._ShrubDomainFemale = np.concatenate((np.zeros([1,self._BarrierLength]), self._ShrubDomainFemale))
                    self._ShrubDomainMale = np.concatenate((np.zeros([1,self._BarrierLength]), self._ShrubDomainMale))
                    self._ShrubPercentCover = np.concatenate((np.zeros([1,self._BarrierLength]), self._ShrubPercentCover))
                    self._BurialDomain = np.concatenate((np.zeros([1,self._BarrierLength]), self._BurialDomain))
                self._DomainWidth = self._DomainWidth + sc # Update width of interior domain
            ...
            elif self._SCRagg < 0: # Negative = erode, remove front row(s) of interior domain
                for d in range(sc):                 
                    newDuneElev = self._InteriorDomain[0,:] # First row of interior will now become new part of active dune field
                    ...
                    # Update shrub domains too
                    self._ShrubDomainFemale = np.delete(self._ShrubDomainFemale,(0), axis=0)
                    self._ShrubDomainMale = np.delete(self._ShrubDomainMale,(0), axis=0)
                    self._ShrubPercentCover = np.delete(self._ShrubPercentCover,(0), axis=0)
        ...
        self._ShrubPercentCover, self._PercentCoverTS, self._ShrubArea = self.CalcPC(ShrubDomainAll, self._PercentCoverTS, self._ShrubArea, self._time_index)
