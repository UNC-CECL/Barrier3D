...
class Barrier3d:
    ...
    def route_overwash(BarrierLength,
        ...
        for d in range(width - 1):
            ...
            for i in range(BarrierLength):
                if Discharge[TS, d, i] > 0:
                    ...
                    if Shrub_ON:
                        """Saline Flooding: Kill all immature (< 1 yr-old) shrubs that have been flooded
        ...
        return SedFluxIn, SedFluxOut, ShrubDomainFemale, ShrubDomainMale, ShrubDomainDead
                ...
                for n in range(numstorm):  # Loop through each individual storm
                    ...
                    for d in range(len(Dow)):  # Loop through each overwashed dune cell
                        for w in range(self._DuneWidth):
                            ...
                            # Alter self._DuneRestart values to model sandbags
                            # If dune is lowered to ~ zero, allow for chance of
                            # regrowth by raising dune height to 5 cm
                            if (
                                self._DuneDomain[self._time_index, Dow[d], w]
                                < self._DuneRestart[w][Dow[d]]
                            ):
                                if self._DuneRestart[w][Dow[d]] < self._Dmax:
                                    self._DuneDomain[
                    ...
                    # represent. of 200-m wide barrier), for inundation regime
