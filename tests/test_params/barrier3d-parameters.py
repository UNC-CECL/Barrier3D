...
TMAX = 50                        # [y] Duration of simulation
StormStart = 2                   # [y] Year when storm can start occurring
BarrierLength = 1000.0           # [m] Static length (alongshore) of island segment
...
BayDepth = 1.5                   # [m] Depth of bay benind island segment
...
HdDiffu = 0.8                    # [m] Dune diffusion parameter (i.e. max height offset between adjacent dune cells)
Dmaxel = 3.0                     # [m] Maximum elevation of dunes
...
Rat = -1.0                       # [m / y] Rate of shoreline reatreat attributed to alongshore transport; (-) = erosion, (+) = accretion
...
surge_tide_m = 0.91              # [m] Mean storm surge + tide water levels
surge_tide_sd = 0.3              # [m] Standard deviation of surge + tide water levels
height_mu = 1.02                 # [m] Mu storm wave height (lognormal distribution)
height_sigma = 0.19              # [m] Sigma storm wave height (lognormal distribution)
period_m = 10.74                 # Mean storm wave period
period_sd = 2.76                 # Standard deviation of storm wave periods
duration_mu = 2.9249             # [hr] Mu storm duration (lognormal distribution)
duration_sigma = 0.638673        # [hr] Sigma storm duration (lognormal distribution)
...
Rin_r = 6.0                      # Run-up regime infiltration rate (volume of overwash flow lost per m cross-shore per time step)
Rin_i = 1.0                      # Inundation regime infiltration rate (volume of overwash flow lost per m cross-shore per time step)
Qs_min = 0.05                    # [m^3 / hr] Minimum discharge needed for sediment transport
threshold_in = 0.25              # [m^3 / hr] Threshold to determine if in inundation regime
Kr = 0.0001                      # Sediment flux constant, run-up regime
Ki = 0.0004                      # Sediment flux constant, inundation regime
Cbb_r = 0.65                     # Coefficient for exponential decay of sediment load entering back-barrier bay in run-up regime
Cbb_i = 0.9                      # Coefficient for exponential decay of sediment load entering back-barrier bay in inundation regime
Qs_bb_min = 0.0001
Cx = 2.0
